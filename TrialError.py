...
# Preload as Sound objects
sounds_mapping = {
    1: pygame.mixer.Sound("sounds/kick-bass.mp3"),
    2: pygame.mixer.Sound("sounds/crash.mp3"),
    3: pygame.mixer.Sound("sounds/snare.mp3"),
    4: pygame.mixer.Sound("sounds/tom-1.mp3"),
    5: pygame.mixer.Sound("sounds/tom-2.mp3"),
    6: pygame.mixer.Sound("sounds/tom-3.mp3"),
    7: pygame.mixer.Sound("sounds/cr78-Cymbal.mp3"),
    8: pygame.mixer.Sound("sounds/cr78-Guiro 1.mp3"),
    9: pygame.mixer.Sound("sounds/tempest-HiHat Metal.mp3"),
    10: pygame.mixer.Sound("sounds/cr78-Bongo High.mp3"),
}

last_notated_count = None
last_notated_time_ms = 0
MIN_NOTE_INTERVAL_MS = 120  # ms debounce for note placement
...
def update_frame():
    global last_notated_count, last_notated_time_ms
    ...
    try:
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                # Classify finger gesture (dynamic)
                finger_gesture_id = 0
                ...
                if hand_label == "Left":
                    ...
                    if is_muted:
                        cv.putText(debug_image, "Muted", (debug_image.shape[1] - 50, 15),
                                   cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 0, cv.LINE_AA)

                # Right Hand = volume adjust
                    ...
                    if is_pinch:
                        ...
                        cv.rectangle(debug_image, (bar_x, bar_y), (bar_x + bar_width, bar_y + bar_height),
                                     (50, 50, 50), -1)
                        cv.rectangle(debug_image, (bar_x, bar_y), (bar_x + filled_width, bar_y + bar_height),
                                     (0, 255, 0), -1)
                ...
                # Log info
                app_logger.info(
                    f"Hand: {hand_label}, Fingers: {finger_count}, "
                    f"Sign: {keypoint_classifier_labels[hand_sign_id]}, "
                    f"Gesture: {point_history_classifier_labels[most_common_fg_id[0][0]]}"
                )
        ...
        # === After processing all hands, trigger sound + notation ===
        if total_finger_count in sounds_mapping:
            ...
            try:
                ...
                if not is_muted and total_finger_count in sounds_mapping:
                    sound = sounds_mapping[total_finger_count]
                    sound.set_volume(last_volume_level / 100.0)  # apply volume
                    sound.play()  # non-blocking
                    app_logger.debug(f"Played sound for gesture {total_finger_count}")
                else:
                    app_logger.debug(f"Skipped sound (muted or invalid) for {total_finger_count}")

            except pygame.error as sound_e:
                app_logger.error(f"Error playing sound {sound_file}: {sound_e}")

            now_ms = int(time.time() * 1000)
            if (total_finger_count != last_notated_count) and \
               (now_ms - last_notated_time_ms >= MIN_NOTE_INTERVAL_MS):
                notation_panel.add_gesture(total_finger_count)
                last_notated_count = total_finger_count
                last_notated_time_ms = now_ms
        else:
            last_notated_count = None
