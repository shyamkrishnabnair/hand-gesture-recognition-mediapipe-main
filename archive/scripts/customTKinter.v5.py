...
recorded_samples = []
gesture_name = ""
gesture_type = ""  # "static", "dynamic", or "multi-hand"
sample_count = 50  # Number of samples to collect per gesture

def start_recording():
    global recording, gesture_name, gesture_type, recorded_samples
    gesture_name = gesture_entry.get().strip()
    gesture_type = gesture_type_var.get()
    if not gesture_name or not gesture_type:
        app_logger.error("Please provide gesture name and type.")
        return
    recording = True
    recorded_samples = []
    app_logger.info(f"Started recording gesture: {gesture_name} ({gesture_type})")
...
def stop_recording():
    ...
    save_gesture_data()
    app_logger.info(f"Stopped recording gesture: {gesture_name}")

def save_gesture_data():
    global recorded_samples, gesture_name, gesture_type
    dataset_path = f"./model/{gesture_type}_classifier/custom_{gesture_name}.csv"
    os.makedirs(os.path.dirname(dataset_path), exist_ok=True)
    with open(dataset_path, "w", newline="") as f:
        writer = csv.writer(f)
        for sample in recorded_samples:
            writer.writerow(sample)
    app_logger.info(f"Saved {len(recorded_samples)} samples to {dataset_path}")

# Load classifiers & labels once
keypoint_classifier = KeyPointClassifier()
point_history_classifier = PointHistoryClassifier()
with open('model/test/keypoint_classifier/keypoint_classifier_label.csv', encoding='utf-8-sig') as f:
...
def refresh():
    stop_camera()
    app.destroy()
    subprocess.Popen([sys.executable, 'customTKinter.v5.py'])
    app_logger.info("GUI refreshed (customTKinter.v5.py restarted)")
...
def update_frame():
    ...
    try:
        if recording and results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                # Assign a new label ID (e.g., increment max existing label)
                    label = len(keypoint_classifier_labels)
                    recorded_samples.append([label] + pre_processed_landmark_list)
                elif gesture_type == "dynamic" and len(pre_processed_point_history_list) == 32:
                    label = len(point_history_classifier_labels)
                    recorded_samples.append([label] + pre_processed_point_history_list)
                elif gesture_type == "multi-hand" and len(results.multi_hand_landmarks) == 2:
                    # Combine landmarks from both hands
                    landmark_list_2 = calc_landmark_list(debug_image, results.multi_hand_landmarks[1])
                    pre_processed_landmark_list_2 = pre_process_landmark(landmark_list_2)
                    combined = pre_processed_landmark_list + pre_processed_landmark_list_2
                    label = len(keypoint_classifier_labels)  # Or create a separate label list
                    recorded_samples.append([label] + combined)

            if len(recorded_samples) >= sample_count:
                stop_recording()
...
# Add to UI setup in customTKinter.v4.py
gesture_frame = ctk.CTkFrame(app, fg_color="#2B2A45")
gesture_frame.pack(pady=10)

gesture_entry = ctk.CTkEntry(gesture_frame, placeholder_text="Enter gesture name", width=200)
gesture_entry.grid(row=0, column=0, padx=10, pady=5)

gesture_type_var = ctk.StringVar(value="static")
gesture_type_menu = ctk.CTkOptionMenu(gesture_frame, values=["static", "dynamic", "multi-hand"], variable=gesture_type_var)
gesture_type_menu.grid(row=0, column=1, padx=10, pady=5)

record_button = ctk.CTkButton(gesture_frame, text="🎥 Record Gesture", command=start_recording)
record_button.grid(row=0, column=2, padx=10, pady=5)

stop_record_button = ctk.CTkButton(gesture_frame, text="⏹ Stop Recording", command=stop_recording, state="disabled")
stop_record_button.grid(row=0, column=3, padx=10, pady=5)
