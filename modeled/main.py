                ...
                for hand_landmarks, handedness in zip(multi_landmarks, multi_handedness):
                    ...
                    if (
                        ...
                        try:
                            ...
                            # ⏱ 1. Compute event time on the global timeline
                            event_time = time.time() - getattr(right_panel, "timeline_start_time")

                            # 🎵 2. Create event object (this matches PDF exactly)
                            event = {
                            ...
                            # 📝 3. ALWAYS push event into the timeline list
                            right_panel.timeline_events.append(event)

                            # 🎼 4. Draw the note using the *same* event + same timeline logic
                            notation_panel.add_timed_event(event, right_panel.timeline_start_time)

                            # 💾 5. If user is recording, ALSO append to recording_data
                            if getattr(right_panel, "is_recording", False):
                                right_panel.recording_data.append(event)
...
right_panel: Any = ctk.CTkFrame(main_frame, width=300)
...
setattr(right_panel, "current_instrument", instrument_ids[current_instrument_index])
# Ensure recording and timeline attributes exist for static analyzers and runtime defaults
setattr(right_panel, "recording_start_time", 0.0)
setattr(right_panel, "recording_data", [])
# Initialize timeline attributes to avoid attribute access errors on CTkFrame
setattr(right_panel, "timeline_events", [])
setattr(right_panel, "timeline_start_time", 0.0)
...
def add_timed_event(self, event):
    ...
    now = time.time() - right_panel.timeline_start_time
    age = now - event["time"]
    ...
    text_id = self.canvas.create_text(x, y, text=symbol, fill="white",
